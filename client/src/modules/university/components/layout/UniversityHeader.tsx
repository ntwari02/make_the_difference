...
import { Notifications as NotificationsIcon, DarkMode as DarkModeIcon, LightMode as LightModeIcon, Logout } from '@mui/icons-material';
...
const UniversityHeader: React.FC = () => {
  ...
  return (
    <AppBar position="sticky" elevation={0} sx={{ borderBottom: (t) => `1px solid ${t.palette.divider}`, zIndex: theme.zIndex.drawer + 1, bgcolor: theme.palette.mode === 'dark' ? '#16213e' : '#ffffff', color: theme.palette.mode === 'dark' ? '#ffffff' : theme.palette.text.primary }}>
      <Toolbar>
        <Typography variant="h6" sx={{ flexGrow: 1 }}>University Portal</Typography>
        <Box>
